...
class VideoProcessor(object):
    def __init__(self, filename, outputname, cache_capacity=10):
        if filename is None:
            self._fps = 25
        ...
        self._output_video_name = "%s.avi" % outputname
...
class VIDDemo(object):
    ...
    def __init__(
        ...
        # for video output
        self.vprocessor = VideoProcessor(None, output_name)
    ...
    def run_on_video(self, video_path):
        ...
        self.vprocessor = VideoProcessor(video_path, self.output_name)
    ...
    def generate_images(self, visualization_results):
        if not os.path.exists(os.path.join(self.output_folder, self.output_name)):
            os.makedirs(os.path.join(self.output_folder, self.output_name))

        for frame_id in range(len(visualization_results)):
            cv2.imwrite(os.path.join(self.output_folder, self.output_name, "%06d.jpg" % frame_id), visualization_results[frame_id])
